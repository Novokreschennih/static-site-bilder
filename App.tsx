...
const App: React.FC = () => {
    ...
    const WelcomeScreen = () => (
        <div className="flex flex-col items-center justify-center min-h-[80vh] text-center p-4">
            ...
            <button onClick={() => document.getElementById('files-upload-welcome')?.click()} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 px-8 rounded-lg text-lg transition-transform transform hover:scale-105">Начать работу</button>
            <input type="file" id="files-upload-welcome" className="hidden" onChange={handleFileChange} multiple {...{ webkitdirectory: "", directory: "" }} />
        </div>
    );
        ...
        <div className="min-h-screen bg-gray-900 text-gray-200 p-4 sm:p-6 lg:p-8">
            <div className="max-w-7xl mx-auto">
                <header className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={() => setIsHelpModalOpen(true)} className="p-2 rounded-full hover:bg-gray-700 transition-colors"> <HelpIcon className="w-6 h-6 text-gray-400"/> </button>
                </header>
                ...
                {appState === 'main' && (
                    files.length === 0 ? (
                        <div onDragOver={e => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={handleDrop} className={`relative border-2 border-dashed rounded-lg p-12 text-center transition-colors duration-300 ${isDragging ? 'border-cyan-500 bg-gray-800' : 'border-gray-600 hover:border-cyan-400'}`}>
                            ...
                            <button onClick={resetState} className="absolute bottom-4 right-4 text-sm text-gray-500 hover:text-gray-300 transition-colors"> ← Назад </button>
                        </div>
                    ) : (
            ...
            <Modal isOpen={isApiKeyModalOpen} onClose={() => setIsApiKeyModalOpen(false)} title="Введите ваш API ключ Google Gemini">
                <div className="space-y-4 text-gray-300">
                    ...
                    <div>
                        <label htmlFor="api-key-input" className="block text-sm font-medium text-gray-300 mb-1"> API ключ </label>
                        <input id="api-key-input" type="password" value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)} className="w-full bg-gray-700 text-white p-2 rounded-md text-sm border border-gray-600 focus:ring-cyan-500 focus:border-cyan-500" placeholder="Введите ваш ключ..." />
                    </div>
                    <p className="text-sm text-gray-400"> Вы можете получить свой ключ в <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline">Google AI Studio</a>. </p>
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-700">
                        <button onClick={() => setIsApiKeyModalOpen(false)} className="px-4 py-2 rounded-md text-sm font-medium text-gray-300 bg-gray-600 hover:bg-gray-500 transition-colors"> Отмена </button>
                        <button onClick={handleSaveApiKey} className="px-4 py-2 rounded-md text-sm font-medium text-white bg-cyan-600 hover:bg-cyan-500 transition-colors"> Сохранить ключ </button>
                    </div>
                </div>
            </Modal>
