    ...
    <div className="space-y-6 text-gray-300 prose prose-invert prose-sm max-w-none">
        ...
        <section>
            ...
            <ul className="list-disc list-inside space-y-2">
                <li><strong>Динамический контент:</strong> Используйте простые метки (плейсхолдеры) вроде <code>{`{{ email_адрес }}`}</code> или <code>[[ Имя пользователя ]]</code> в своих HTML-файлах. Перед упаковкой вы сможете легко заменить их на реальные данные, не трогая код. Идеально для шаблонных сайтов!</li>
                <li><strong>Глобальные скрипты:</strong> Вставьте код Яндекс.Метрики или Google Analytics один раз, и он автоматически добавится на все страницы вашего сайта.</li>
        ...
        <section>
            ...
            <h4 className="text-lg font-semibold text-white mt-4">Шаг 2: Настройка плейсхолдеров</h4>
            <p>Плейсхолдеры — это метки в вашем коде, которые можно легко заменить на нужные данные. Используйте их для информации, которая может меняться: заголовки, контактные данные, ссылки и т.д.</p>
            <p><strong>Формат:</strong> Плейсхолдеры должны быть в формате <code>[[ что-то ]]</code> или <code>{`{{ что-то }}`}</code>.</p>
            <p className="font-semibold mt-2">Как настроить:</p>
            <ol className="list-decimal list-inside space-y-2">
                <li><strong>Выберите страницу:</strong> Нажмите на карточку HTML-файла, который хотите настроить.</li>
                <li><strong>Заполните поля:</strong> В правой панели "Настройки страницы" появятся поля для каждого найденного плейсхолдера. Введите туда нужные значения.</li>
            ...
            <p>Если на выбранной странице нет плейсхолдеров, вы увидите соответствующее сообщение.</p>
            ...
            <ol className="list-decimal list-inside space-y-2">
                ...
                <li>Приложение выполнит всю магию:
                     <ul className="list-disc list-inside ml-6 mt-2 space-y-1">
                        ...
                        <li>Автоматически обновит все внутренние ссылки (<code>&lt;a href="..."&gt;</code>, <code>&lt;img src="..."&gt;</code>) на новые имена файлов.</li>
